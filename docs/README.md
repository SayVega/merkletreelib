...
Generates a Merkle inclusion proof for a given leaf. It returns a vector with the sibling hashes and direction (left/right) required to reconstruct the Merkle tree (starting from leaf to root). If `target` was not found in the tree returns None.
The leaf is not included in the proof.

### verify_proof
Given a leaf, a Merkle proof, and a root hash, this function reconstructs the Merkle root. Returns `true` if the proof is valid and reconstruct `root_hash`, otherwise returns `false`.
